...
const LINKED_DOMAINS = [
    'site-a.com',        // Replace with your actual domains
    'site-b.com',        // e.g. 'mybrand.com', 'mybrandshop.com'
];
...
// ─── Cross-Domain: Read incoming consent from URL ──────────────────────────
function applyIncomingConsent() {
    ...
    const GCS_MAP = {
        ...
        'G100': { status: 'rejected',  categories: { functional: false, analytics: false, performance: false, advertising: false } },
        'G101': { status: 'custom',    categories: { functional: true,  analytics: true,  performance: false, advertising: false } },
        'G110': { status: 'custom',    categories: { functional: true,  analytics: false, performance: false, advertising: true  } }
    };

    const consentData = GCS_MAP[gcs];
    ...
    console.log('Cross-domain consent applied:', consentData.status);
}

// ─── Cross-Domain: Tag outbound links with consent state ───────────────────
function tagOutboundLinks() {
    ...
    let gcs;
    try {
        const data = JSON.parse(consentCookie);
        gcs = data.status === 'accepted' ? 'G111'
            : data.status === 'rejected' ? 'G100'
            : (data.categories?.analytics && data.categories?.advertising) ? 'G111'
            : (data.categories?.analytics) ? 'G101'
            : (data.categories?.advertising) ? 'G110'
            : 'G100';
    } catch(e) { return; }
